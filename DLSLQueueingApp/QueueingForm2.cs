...
{
    public partial class QueueingForm2 : Form
    {
        public QueueingForm2()
        {
            InitializeComponent();
        }
        int queueno;
        ...
        public void loadData()
        {
            ...
            String query = "SELECT queue_no, queueing_status, service_type, service_lane FROM service WHERE cashier_number=2 AND queueing_status='PENDING' UNION SELECT queue_no, queueing_status, service_type, service_lane FROM service_manual WHERE cashier_number = 2 AND queueing_status='PENDING' ORDER BY service_lane = 'PRIORITY' DESC, queue_no ASC; ";
        ...
        {
            String connection = "server=localhost;user id=root; password=root;database=dlsl_app"; // Para magstart yung mysql
            String query = "UPDATE cashier SET current_queue_number ='" + queueno + "' where cashier_number=2;";
        ...
        private void nextQueueBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                String connection = "server=localhost;user id=root; password=root;database=dlsl_app"; // Para magstart yung mysql
                String query = "UPDATE service_manual SET queueing_status = 'DONE' where queue_no='" + queueno + "' AND cashier_number=2;";
                String query2 = "UPDATE service SET queueing_status = 'DONE' where queue_no='" + queueno + "' AND cashier_number=2;";
                MySqlConnection con = new MySqlConnection(connection);
                MySqlCommand cmd = new MySqlCommand(query, con);
